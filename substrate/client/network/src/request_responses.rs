...
use libp2p::{
	core::{transport::PortUse, Endpoint, Multiaddr as Libp2pMultiaddr},
...
pub enum RequestFailure {
	...
	Obsolete,
	#[error("Problem on the network: {}", display_network_error(.0, .1))]
	Network(OutboundFailure, Option<Multiaddr>),
}

fn display_network_error(error: &OutboundFailure, peer_addr: &Option<Multiaddr>) -> String {
	match peer_addr {
		Some(addr) => format!("{} (peer: {})", error, addr),
		None => format!("{}", error),
	}
}

impl RequestFailure {
	/// Returns the peer address associated with this failure, if known.
	/// This is populated for `Network` failures when the peer's address could be determined.
	pub fn peer_address(&self) -> Option<&Multiaddr> {
		match self {
			RequestFailure::Network(_, addr) => addr.as_ref(),
			_ => None,
		}
	}
}
...
pub struct RequestResponsesBehaviour {
	...
	/// Tracks the last known address for each connected peer.
	/// Used to provide peer address info in outbound failure errors.
...
impl NetworkBehaviour for RequestResponsesBehaviour {
	...
	fn handle_pending_inbound_connection(
		...
		_local_addr: &Libp2pMultiaddr,
		_remote_addr: &Libp2pMultiaddr,
	...
	fn handle_pending_outbound_connection(
		...
		_addresses: &[Libp2pMultiaddr],
		_effective_role: Endpoint,
	) -> Result<Vec<Libp2pMultiaddr>, ConnectionDenied> {
		Ok(Vec::new())
	}
	...
	fn handle_established_inbound_connection(
		...
		peer: PeerId,
		local_addr: &Libp2pMultiaddr,
		remote_addr: &Libp2pMultiaddr,
	...
	fn handle_established_outbound_connection(
		...
		// Store the peer's address for error reporting on failures
		self.peer_addresses.insert(peer, addr.clone().into());
	...
	fn on_swarm_event(&mut self, event: FromSwarm) {
		// Clean up peer address tracking when all connections to a peer are closed
		if let FromSwarm::ConnectionClosed(ref closed) = event {
			if closed.remaining_established == 0 {
				self.peer_addresses.remove(&closed.peer_id);
			}
		}
			...
			if self.periodic_request_check.poll_tick(cx).is_ready() {
				self.pending_requests.retain(|id, req| {
					...
					if elapsed > *request_timeout {
						...
						if let Some(response_tx) = req.response_tx.take() {
							if response_tx.send(Err(RequestFailure::Network(OutboundFailure::Timeout, None))).is_err() {
								log::debug!(
									target: LOG_TARGET,
				...
				'poll_protocol: while let Poll::Ready(ev) = behaviour.poll(cx) {
					...
					match ev {
						...
						request_response::Event::OutboundFailure {
							...
							// Look up the peer's address for inclusion in the error
							let peer_addr = self.peer_addresses.get(&peer).cloned();
								...
								.remove(&(protocol.clone(), request_id).into())
							{
								Some(PendingRequest {
									...
									if response_tx
										.send(Err(RequestFailure::Network(error.clone(), peer_addr.clone())))
										.is_err()
									{
										log::debug!(
											target: LOG_TARGET,
							...
							let out = Event::RequestFinished {
								...
								protocol: protocol.clone(),
								duration: started.elapsed(),
								result: Err(RequestFailure::Network(error, peer_addr)),
							};
...
mod tests {
	...
	fn build_swarm(
		list: impl Iterator<Item = ProtocolConfig>,
	) -> (Swarm<RequestResponsesBehaviour>, Libp2pMultiaddr) {
		...
		let listen_addr: Libp2pMultiaddr = format!("/memory/{}", rand::random::<u64>()).parse().unwrap();
	...
	async fn max_response_size_exceeded() {
		...
		match response_receiver.unwrap().await.unwrap().unwrap_err() {
			RequestFailure::Network(OutboundFailure::Io(_), _) => {},
	...
	async fn enforce_outbound_timeouts() {
		...
		// Expect the timeout.
		match response_receiver.unwrap().await.unwrap().unwrap_err() {
			RequestFailure::Network(OutboundFailure::Timeout, _) => {},
			request_failure => panic!("Unexpected failure: {request_failure:?}"),
		}
	}
}
